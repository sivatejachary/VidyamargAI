...
logger = logging.getLogger("packages.core-lib.database")
...
class DatabaseManager:
    ...
    def __init__(self, database_url: str, echo: bool = False):
        # Convert traditional postgresql:// URL to asyncpostgresql:// if necessary
        ...
        self.engine = create_async_engine(
            self.database_url,
            echo=echo,
            pool_size=20,            # Max active connections in the pool
            max_overflow=10,        # Temp overflow connections allowed
            pool_recycle=1800,      # Recycle connections after 30 mins
            pool_pre_ping=True,      # Heartbeat check before using connection
            json_serializer=lambda obj: obj, # Use raw orjson inside asyncpg where possible
        )
