...
export default function Certificates({ certificates }: CertificatesProps) {
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-[24px] font-black text-slate-905 dark:text-white leading-tight">Certificates</h2>
        ...
        {certificates.length > 0 ? (
          <div className="flex flex-col gap-3">
            {certificates.map((c, i) => (
              <div 
                key={`db-cert-${i}`} 
                className="p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    ...
                    <span className="text-[9px] text-slate-450 font-mono block mt-1 uppercase font-bold">
                ...
                <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto justify-end">
                  <button
                    ...
                    className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 rounded-lg text-[10px] font-bold cursor-pointer"
                  >
                    Verify Code
                  </button>
                  <button
                    onClick={() => alert("LinkedIn sharing initialized. Certificate successfully shared!")}
                    className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-705 text-white rounded-lg text-[10px] font-bold cursor-pointer"
                  >
                    Share LinkedIn
                  </button>
                </div>
