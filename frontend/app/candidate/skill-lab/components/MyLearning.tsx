...
export default function MyLearning({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-[24px] font-black text-slate-905 dark:text-white leading-tight">My Learning</h2>
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800/40 pb-4">
                  <div>
                    <span className="px-2 py-0.5 rounded text-[8px] font-bold font-mono bg-indigo-100 text-indigo-650 dark:bg-indigo-950/30 dark:text-indigo-400 uppercase tracking-wider">
                      Active Course
                    </span>
                    <h3 className="text-base font-black text-slate-800 dark:text-white mt-2 leading-snug">{courseObj.title}</h3>
                    <span className="text-[10px] text-slate-500 mt-1 block font-medium">Instructor: {courseObj.instructor}</span>
                  </div>
                  
                  <div className="text-right">
                    <span className="text-sm font-black text-emerald-500 block">{Math.round(enroll.progress)}% Complete</span>
                    <span className="text-[9px] text-slate-400 font-bold block mt-0.5">Overall Progress</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl">
                    <span className="text-[9px] uppercase font-bold text-slate-400">Current Module</span>
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl">
                    <span className="text-[9px] uppercase font-bold text-slate-400">Next Assessment</span>
