...
export default function Hackathons() {
  ...
  if (!assignment) {
    return (
      <div className="w-full min-h-screen bg-background dark:bg-background p-6 font-sans text-gray-800 dark:text-gray-100 flex flex-col items-center justify-center gap-4 transition-colors duration-300">
        <div className="w-16 h-16 rounded-full bg-purple-100 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-900/25 flex items-center justify-center text-indigo-500 shadow-sm">
  ...
  return (
    <div className="w-full min-h-screen bg-background dark:bg-background p-6 font-sans text-gray-800 dark:text-gray-100 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm mb-6 flex flex-col gap-5">
        <div>
          <div className="flex items-center gap-2.5">
            <h2 className="text-base font-bold text-gray-990 dark:text-white">AIML Intern - Hackathon (5 Jun'26)</h2>
            <span className="text-10 font-bold px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30">
              Live
            </span>
          </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="border-t border-gray-100 dark:border-gray-855 pt-3">
            <h4 className="text-10 uppercase font-bold tracking-wider text-gray-400 mb-3">Team Members ({members.length})</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {members.map((m) => (
                ...
                >
                  <div className="w-9 h-9 rounded-xl bg-purple-50 dark:bg-purple-950 text-indigo-500 dark:text-purple-300 flex items-center justify-center font-bold text-xs shrink-0 border border-purple-100/40 dark:border-purple-900/10">
                    {m.initials}
                  </div>
                  <div className="overflow-hidden leading-tight">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-9 font-bold text-gray-400">{m.role}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-855 pb-3 mb-4 mt-8">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-card hover:bg-gray-50 dark:hover:bg-gray-900 text-xs font-bold transition-all text-gray-600 dark:text-gray-300 cursor-pointer"
        >
          ...
          <button
            ...
            className={`px-4.5 py-2.5 rounded-xl text-xs font-bold transition-all border whitespace-nowrap cursor-pointer flex items-center gap-1.5 ${
              activeTab === q.id
                ...
                : "bg-white dark:bg-card border-gray-200 dark:border-gray-800 text-gray-500 hover:text-gray-900 dark:hover:text-white"
            }`}
          >
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
        <div className="flex justify-between items-start border-b border-gray-100 dark:border-gray-855 pb-3.5">
          <div>
            ...
            <span className="text-10 text-gray-400 mt-0.5 block">Estimated duration: 3 hours</span>
          </div>
          {activeQuestion.isCompleted && (
            <span className="text-10 font-bold px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30 flex items-center gap-1">
        ...
        <div className="border-t border-gray-100 dark:border-gray-855 pt-4 mt-2 flex justify-end">
          <button 
            ...
            className="flex items-center gap-1 px-4 py-2 rounded-xl bg-indigo-500 hover:bg-indigo-650 text-xs font-bold text-white transition-all shadow-sm cursor-pointer"
          >
      ...
      {showEditorModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-3xl p-6 w-full max-w-3xl shadow-2xl flex flex-col gap-4 max-h-95-vh overflow-y-auto font-sans">
            ...
            <div className="text-11 text-gray-500 dark:text-gray-400 leading-relaxed bg-gray-50 dark:bg-gray-950/40 p-3.5 rounded-2xl border border-gray-200 dark:border-gray-850">
            ...
            <div className="flex flex-col gap-1.5 mt-1.5">
              <label className="text-10 font-bold uppercase tracking-wider text-gray-500">Python 3 code editor</label>
              <textarea
                value={code}
                onChange={(e) => setCode(e.target.value)}
                rows={10}
                className="w-full font-mono bg-gray-50 dark:bg-card border border-gray-200 dark:border-gray-800 rounded-2xl p-4 text-gray-700 dark:text-muted-foreground text-11 leading-relaxed focus:outline-none focus:border-indigo-500"
              />
            </div>

            {/* Action buttons */}
            <div className="flex justify-between items-center mt-1">
              <span className="text-10 text-gray-500 font-semibold">Proctoring agent is active. Avoid tab switching.</span>
              
              <div className="flex gap-3">
                <button
                  onClick={handleRunCode}
                  disabled={runLoading || runSuccess}
                  className="flex items-center gap-1.5 px-4.5 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-650 text-xs font-bold text-white transition-all cursor-pointer shadow-sm disabled:bg-gray-800 disabled:text-gray-500"
                >
            ...
            {runLogs.length > 0 && (
              <div className="bg-gray-50 dark:bg-black rounded-2xl border border-gray-200 dark:border-gray-850 p-4 font-mono text-10 text-gray-500 dark:text-gray-400 space-y-1 max-h-40 overflow-y-auto">
                {runLogs.map((log, idx) => (
      ...
      {showPdfModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-3xl p-6 w-full max-w-lg shadow-2xl flex flex-col gap-4">
            ...
            <button 
              onClick={() => setShowPdfModal(false)}
              className="w-full py-2.5 bg-indigo-500 hover:bg-indigo-650 text-white rounded-xl text-xs font-bold transition-all text-center cursor-pointer"
            >
              Close Spec Viewer
            </button>
          </div>
