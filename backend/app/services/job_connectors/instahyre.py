...
def fetch(queries: List[str]) -> List[LiveJob]:
    ...
    for query in queries[:3]:
        ...
        for r in results:
            ...
            title = re.sub(r'\s*-\s*Instahyre.*', '', raw_title, flags=re.IGNORECASE).strip()
            ...
            jobs.append(LiveJob(
                ...
                source=SOURCE,
                description=f"Opportunity found on Instahyre:\n\n{snippet}\n\nApply on Instahyre: {url}",
                work_mode=work_mode,
            ))
    logger.info(f"Instahyre connector returned {len(jobs)} jobs")
    return jobs
