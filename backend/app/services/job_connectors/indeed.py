...
def fetch(queries: List[str]) -> List[LiveJob]:
    ...
    for query in queries[:3]:
        ...
        for r in results:
            ...
            title = re.sub(r'\s*-\s*Indeed.*', '', raw_title, flags=re.IGNORECASE).strip()
            if " - " in title:
                parts = title.split(" - ", 1)
                title = parts[0].strip()
            ...
            jobs.append(LiveJob(
                ...
                source=SOURCE,
                description=f"Opportunity found on Indeed:\n\n{snippet}\n\nApply on Indeed: {url}",
                work_mode=work_mode,
            ))
    logger.info(f"Indeed connector returned {len(jobs)} jobs")
    return jobs
