...
def fetch(queries: List[str]) -> List[LiveJob]:
    ...
    for query in queries[:3]:
        search_query = f'{query} site:hirist.tech/jobs/ OR site:hirist.com/job/ "India"'
        ...
        for r in results:
            ...
            title = re.sub(r'\s*-\s*Hirist.*', '', raw_title, flags=re.IGNORECASE).strip()
            if " - " in title:
                parts = title.split(" - ", 1)
                title = parts[0].strip()
            ...
            jobs.append(LiveJob(
                ...
                source=SOURCE,
                description=f"Opportunity found on Hirist:\n\n{snippet}\n\nApply on Hirist: {url}",
                work_mode=work_mode,
            ))
    logger.info(f"Hirist connector returned {len(jobs)} jobs")
    return jobs
