"""
Recruitee Adapter — Auto Apply implementation for Recruitee.
"""
import logging
...
class RecruiteeAdapter(BaseApplicationAdapter):
    adapter_version: str = "recruitee:1.0"
    platform_key: str = "recruitee"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on Recruitee."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to Recruitee file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on Recruitee."""
        filled = 0
        fields = [
    ...
    async def fill_education(self, page, profile: dict) -> bool:
        """Fill education fields on Recruitee."""
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from Recruitee."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on Recruitee."""
