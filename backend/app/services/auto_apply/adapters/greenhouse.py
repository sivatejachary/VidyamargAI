"""
Greenhouse Adapter — Auto Apply implementation for Greenhouse.
"""
import logging
...
class GreenhouseAdapter(BaseApplicationAdapter):
    adapter_version: str = "greenhouse:1.0"
    platform_key: str = "greenhouse"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on Greenhouse."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to Greenhouse file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on Greenhouse."""
        filled = 0
        fields = [
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from Greenhouse."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on Greenhouse."""
