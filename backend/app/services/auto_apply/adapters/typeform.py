"""
Typeform Adapter — Auto Apply implementation for Typeform.
"""
import logging
...
class TypeformAdapter(BaseApplicationAdapter):
    adapter_version: str = "typeform:1.0"
    platform_key: str = "typeform"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on Typeform."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to Typeform file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on Typeform."""
        filled = 0
        fields = [
    ...
    async def fill_education(self, page, profile: dict) -> bool:
        """Fill education fields on Typeform."""
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from Typeform."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on Typeform."""
