"""
GenericWebsite Adapter — Auto Apply implementation for GenericWebsite.
"""
import logging
...
class GenericWebsiteAdapter(BaseApplicationAdapter):
    adapter_version: str = "generic:1.0"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on GenericWebsite."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to GenericWebsite file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on GenericWebsite."""
        filled = 0
        fields = [
    ...
    async def fill_education(self, page, profile: dict) -> bool:
        """Fill education fields on GenericWebsite."""
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from GenericWebsite."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on GenericWebsite."""
