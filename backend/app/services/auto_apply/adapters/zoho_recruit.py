"""
ZohoRecruit Adapter — Auto Apply implementation for ZohoRecruit.
"""
import logging
...
class ZohoRecruitAdapter(BaseApplicationAdapter):
    adapter_version: str = "zoho_recruit:1.0"
    platform_key: str = "zoho_recruit"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on ZohoRecruit."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to ZohoRecruit file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on ZohoRecruit."""
        filled = 0
        fields = [
    ...
    async def fill_education(self, page, profile: dict) -> bool:
        """Fill education fields on ZohoRecruit."""
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from ZohoRecruit."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on ZohoRecruit."""
