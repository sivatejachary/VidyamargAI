"""
OracleHCM Adapter — Auto Apply implementation for OracleHCM.
"""
import logging
...
class OracleHCMAdapter(BaseApplicationAdapter):
    adapter_version: str = "oracle_hcm:1.0"
    platform_key: str = "oracle_hcm"
    ...
    async def create_account(self, context, profile: dict, platform: str) -> dict:
        """Create new account on OracleHCM."""
    ...
    async def upload_resume(self, page, resume_bytes: bytes, filename: str) -> bool:
        """Upload resume to OracleHCM file upload field."""
        try:
    ...
    async def fill_personal_details(self, page, profile: dict) -> bool:
        """Fill personal details on OracleHCM."""
        filled = 0
        fields = [
    ...
    async def fill_education(self, page, profile: dict) -> bool:
        """Fill education fields on OracleHCM."""
    ...
    async def capture_confirmation(self, page) -> dict:
        """Capture confirmation from OracleHCM."""
        try:
    ...
    async def detect_verification_required(self, page) -> Optional[str]:
        """Detect OTP/CAPTCHA/email verification on OracleHCM."""
