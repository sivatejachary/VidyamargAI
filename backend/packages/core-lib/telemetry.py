...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Inject standard log exceptions if present
        if record.exc_info:
            log_data["exception"] = self.formatException(record.exc_info)
            
        # Inject custom execution context (e.g. user_id, session_id) if attached
...
def configure_telemetry(service_name: str, debug_mode: bool = False):
    ...
    logger = logging.getLogger("packages.core-lib.telemetry")
